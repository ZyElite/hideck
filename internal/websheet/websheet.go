...
func callbackHTML() string {
	return `<!doctype html><html><body><script>try{window.parent.postMessage({type:"hideck-websheet-callback"},"*")}catch(_){}</script>Carrier flow returned to HiDeck.</body></html>`
}
