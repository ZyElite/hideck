...
func TestDecodeDeliverTPDUClassifiesBinaryOmaCP(t *testing.T) {
	deliver, err := tpdu.NewDeliver(tpdu.WithOA(tpdu.NewAddress(tpdu.FromNumber("85075"))))
	if err != nil {
		t.Fatal(err)
	}
	deliver.SetPID(0)
	deliver.SetDCS(0x04)
	deliver.SetUDH(tpdu.UserDataHeader{
		{ID: 0x05, Data: []byte{0x0b, 0x84, 0x23, 0xf0}},
	})
	deliver.SetUD(buildTestWBXML())
	raw, err := deliver.MarshalBinary()
	if err != nil {
		t.Fatal(err)
	}
	_, text, _, _, err := DecodeDeliverTPDU(raw)
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(text, "[OMA CP 运营商配置短信]") ||
		!strings.Contains(text, "NAP-ADDRESS") {
