...
test('login surfaces config and environment password remediation', () => {
  ...
  assert.match(login, /systemService\.changePassword\(passwordChangeForm\.value\)/)
  assert.match(login, /auth\.applyToken\(result\.data\.token\)/)
  assert.match(login, /old_password: form\.value\.password/)
  assert.match(login, /v-model="passwordChangeOpen"/)
  assert.match(login, /autocomplete="current-password"/)
  assert.match(login, /autocomplete="new-password"/)
  assert.doesNotMatch(login, /\/settings\?focus=password/)
...
test('settings disables ineffective changes for environment managed passwords', () => {
  ...
  assert.doesNotMatch(settings, /bcrypt|哈希安全写入/)
})

test('credential APIs require typed login state and return replacement sessions', () => {
  assert.match(authStore, /isCredentialStatus/)
  assert.match(authStore, /isPasswordValidationError/)
  assert.match(authStore, /data\?\.code === 'invalid_password'/)
